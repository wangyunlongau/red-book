...
//    if (n == 0 || n == 1) return n
//
//    def go(n: Int, nMinusTwo: Int, nMinusOne: Int): Int = {
//      if (n == 0) nMinusOne
//      else go(n - 1, nMinusOne, nMinusOne+nMinusTwo)
//    }
//
//    // e.g. if index is 4, we need to do addition 3 times to get the fib number
//    val numOfAddition = n - 1
//
//    go(numOfAddition, 0, 1)
    @annotation.tailrec
...
//    def go(n: Int, as: Array[A], gt: (A,A) => Boolean): Boolean = {
//      if (n == as.length - 1) true
//      else if (gt(as(n), as(n+1))) go(n+1, as, gt)
//      else false
//    }
//    go(0, as, gt)
